...
> 💬 **폰에서 대화 입력(전송)까지 됩니다** — 폴더 열고 메시지를 보내면 PC의 Claude가 이어서 답해요. (보기·알림·권한 승인도 전부 가능) 단, 일부 *조직 제한 계정*은 추론이 막혀 전송이 안 될 수 있어요 → [한계](#-한계).
...
- 💬 **폰에서 대화 입력/전송** — 폴더 열고 메시지 보내면 Claude가 이어서 응답 (`--continue`)
...
- **전송(대화 입력)은 계정이 추론을 허용해야 작동합니다.** 대부분의 개인 계정은 잘 되지만, 일부 **조직(회사/팀) 제한 계정**은 `403: organization has disabled ... Use an Anthropic API key` 로 막힐 수 있어요.
  - 막혀도 **보기 · 알림 · 권한 승인 · 선택지 응답**은 정상이에요 (이미 돌아가는 세션에 "답"만 주는 것이라 OK).
  - 막히면 **Anthropic API 키**(유료) 또는 **비조직 개인 계정**으로 `claude` 로그인하면 전송이 됩니다.
- 전송은 그 폴더 대화를 `--continue`로 이어가요 — **기록이 있는 폴더**에 보내야 자연스럽게 이어집니다. (빈 폴더면 첫 메시지는 PC에서 한 번 시작)
